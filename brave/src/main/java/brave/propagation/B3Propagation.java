...
public final class B3Propagation<K> implements Propagation<K> {
  ...
  static final class B3Injector<C, K> implements TraceContextInjector<C> {
    ...
    @Override public void inject(TraceContext traceContext, C carrier) {
      setter.put(carrier, propagation.traceIdKey, traceContext.traceId().toString());
    ...
    @Override public TraceContext extract(C carrier) {
      ...
      if (traceIdString != null) {
        TraceId traceId = TraceId.create(
            ...
            lowerHexToUnsignedLong(traceIdString)
        );
        ...
        TraceId traceId = TraceId.create(traceId128Bit ? Platform.get().randomLong() : 0, spanId);
        return result.traceId(traceId).spanId(spanId).build();
      }
